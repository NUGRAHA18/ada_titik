...
18. [Me — `/api/me`](#18-me--apime)
19. [Health Check](#19-health-check)
20. [Panduan Postman](#20-panduan-postman)
21. [Aturan Validasi Lengkap](#21-aturan-validasi-lengkap)
22. [Changelog](#22-changelog)
23. [Ringkasan Endpoint](#23-ringkasan-endpoint)

---

## 1. Gambaran Umum

**Titik Baik** adalah platform *social donation mapping* berbasis lokasi. API ini menyediakan seluruh backend untuk mengelola titik bantuan, autentikasi pengguna, dokumentasi distribusi, sistem rating, pelaporan fraud, modul komunitas, dan moderasi admin.
...
### 9.6 Update Status Titik Bantuan (legacy)

> **Sejak v3.2 (flow donasi baru):** transisi status disarankan otomatis lewat
> `accept` / `complete` per participant (lihat 9.7–9.13). Endpoint ini tetap
> tersedia untuk backward-compat dan tutup-manual oleh komunitas.
...
### 9.7 Donatur Tekan "Berangkat"

> **Baru di v3.2.** Mencatat sinyal Berangkat dari donatur (tidak lagi mengubah
> status titik secara langsung). Detail lengkap & rasional di `flow_donation_done.md`.

```
POST /api/donations/:pointId/participants
```

**Autentikasi:** Bearer Token (**donatur**)
...
Idempotent: jika donatur sudah Berangkat ke titik ini, response 200 dengan
row yang sama (tidak duplikat).
...
**Side effects:** Notifikasi `donator_departed` dibuat untuk owner komunitas.

---

### 9.8 Donatur Batalkan Berangkat

```
DELETE /api/donations/:pointId/participants/me
```

**Autentikasi:** Bearer Token (**donatur**). Hanya boleh untuk state
`requested` / `accepted` (tidak untuk `completed`).

---

### 9.9 Donatur Cek Partisipasi Sendiri

```
GET /api/donations/:pointId/participants/me
```

**Autentikasi:** Bearer Token (semua role). Return `{ "data": null }` bila
belum pernah Berangkat.

---

### 9.10 Komunitas Lihat Daftar Donatur Titik

```
GET /api/donations/:pointId/participants?state=requested
```

**Autentikasi:** Bearer Token (**komunitas — pemilik**). Query `state` opsional:
...
### 9.11 Komunitas Accept (Bulk)

```
PATCH /api/donations/:pointId/participants/accept
```

**Autentikasi:** Bearer Token (**komunitas — pemilik**)

```json
{ "donator_ids": ["uuid1", "uuid2"] }
```

Hanya row dengan `state='requested'` yang berubah ke `accepted`. Memicu
recompute aggregates titik (`status`, `urgency`, `collected_amount`) dan
notifikasi `participant_accepted` ke tiap donatur.
...
### 9.12 Komunitas Complete (Bulk) + Geo-Fencing
...
- Geo-fencing ≤ 100 m wajib (lokasi komunitas).
- `per_donator_amount` opsional → mengisi `contribution_amount`.
- **+50 Poin Donatur** per donatur (audit di `donator_points_log`).
- Memicu recompute aggregates + notifikasi `participant_completed`; jika
  urgency berubah → notifikasi `urgency_changed` tambahan.
...
| Kode | Kondisi |
|---|---|
| `400` | `donator_ids` kosong, atau `user_lat`/`user_lng` tidak lengkap |
| `403` | Bukan pemilik titik, atau di luar radius geo-fencing |
| `404` | Titik tidak ditemukan |

---

### 9.13 Komunitas Update Progres
...
Minimal satu field wajib. Memicu recompute urgency + notifikasi
`progress_updated` ke semua participant aktif. Jika urgency berubah,
tambahan notifikasi `urgency_changed`.

**Formula urgency otomatis (`src/utils/urgency.js`):**

| Progress (`collected/goal`) | Urgency |
|---|---|
| `< 0.33` atau `goal <= 0` | `Mendesak` |
| `< 0.80` | `Normal` |
| `>= 0.80` | `Rendah` |
...
> **Diubah di v3:** response `/nearby` berbentuk notifikasi (`title`, `subtitle`, `type`).
> **Baru di v3.2:** modul **event-driven** dengan tabel `notifications` + CRUD
> per-user. Realtime push via Supabase Realtime (lihat `notifications_done.md`).

### 15.0 Tipe Notifikasi (kontrak `type`)

| `type` | Trigger | Penerima |
|---|---|---|
| `donator_departed` | Donatur tekan / batalkan Berangkat | Owner komunitas |
| `participant_accepted` | Komunitas accept donatur | Donatur ybs. |
| `participant_completed` | Komunitas complete donatur (+poin) | Donatur ybs. |
| `progress_updated` | Komunitas update goal/collected | Semua participant aktif |
| `urgency_changed` | Urgency titik berubah otomatis | Semua participant aktif |
| `post_liked` | User like postingan | Author post |
| `post_commented` | User komentari postingan | Author post |

> Notifikasi tidak dikirim ke diri sendiri (actor == penerima → di-skip oleh service).

---

### 15.1 List Notifikasi Saya
...
|---|---|---|---|
| `page` | integer | `1` | Nomor halaman |
| `limit` | integer | `20` | Maks 50 |
| `unread` | boolean | — | Jika `true`, hanya tampilkan yang belum dibaca |
  ...
  "data": [
    {
      ...
      "body": "Titik \"Bantuan Pangan\" — Anda di-accept oleh komunitas",
...
`unread_count` selalu mencerminkan total belum-dibaca user, terlepas dari filter.

---

### 15.2 Tandai Satu Notifikasi Dibaca

```
PATCH /api/notifications/:id/read
```

**Autentikasi:** Bearer Token. `404` jika id bukan milik user.
...
### 15.3 Tandai Semua Notifikasi Dibaca
...
### 15.5 Notifikasi Bantuan Terdekat (legacy `/nearby`)
...
- **Semua endpoint wajib JWT.** `userId` selalu diambil dari `req.user.userId` (payload JWT) — FE tidak perlu kirim `userId`.
- **Pagination konsisten** dengan endpoint community lain: `?page=` (default 1), `?limit=` (default 10, maks 50).
- **Tanggal selalu ISO8601 (UTC)** sehingga FE bisa langsung memformat `timeAgo`.
- **`post_id` & `comment_id` selalu integer** (kolom `SERIAL` di Postgres) — konsisten dengan endpoint community lain.
- Response menyertakan **`post_snapshot`** agar FE bisa langsung render tanpa fetch tambahan per item.

---

### 18.1 Postingan Saya

Mengambil daftar post yang **dibuat user** (urut terbaru). Hanya akan berisi data jika user adalah role `komunitas` (karena `POST /api/community/posts` saat ini terbatas untuk role komunitas).
...
```
GET /api/me/community/posts?page=1&limit=10
Authorization: Bearer eyJ0eXAi...
```
...
```json
{
  "pagination": { "total": 5, "total_pages": 1, "current_page": 1, "limit": 10 },
  "data": [
    {
      "id": 12,
      "content": "Update: distribusi paket pangan tahap 2 selesai...",
...
> Field `liked_by_me` di sini mencerminkan apakah **author** juga menge-like postnya sendiri. Biasanya `false`, tapi tetap disertakan agar shape konsisten dengan `GET /api/community/posts`.

---

### 18.2 Like yang Saya Berikan

Mengambil daftar post yang **di-like user**, beserta snapshot post untuk render langsung di FE. Urut berdasarkan waktu like (terbaru).
...
- `created_at` (top-level) = **waktu user menge-like post tersebut**.
- `post_snapshot.created_at` = waktu post asli dibuat.
- Jika sebuah post dihapus (sehingga `post_likes` ON DELETE CASCADE), entry like-nya hilang juga — list ini selalu konsisten.

---

### 18.3 Komentar yang Saya Tulis

Mengambil daftar komentar yang **ditulis user**, urut terbaru. Disertakan ringkasan post tempat komentar berada.
...
```json
{
  "pagination": { "total": 12, "total_pages": 2, "current_page": 1, "limit": 10 },
  "data": [
    {
      "comment_id": 7,
      "post_id": 12,
...
- `comment_id` & `post_id` keduanya integer.
- `created_at` = waktu komentar ditulis.
- Jika post dihapus, entry komentar terkait juga hilang (ON DELETE CASCADE).

---

### Pemetaan ke `UserActivityScreen` (FE)
...
3. Aktifkan environment **"Titik Baik – Local (Yogyakarta)"** di dropdown kanan atas
...
`requirement_notification.md`, dan `requirement_chat.md`. Migration:
`database/migration_v5.sql`. Penanda implementasi lengkap untuk FE:
`flow_donation_done.md`, `notifications_done.md`, `chat_realtime_done.md`.

#### DB Schema

- Tabel baru: `donation_participants`, `notifications`, `donator_points_log`.
- Kolom baru: `users.points INT NOT NULL DEFAULT 0`.
- Default `donation_points.urgency` → `'Mendesak'` (sebelumnya `'Normal'`).
- Realtime publication ditambah: `chat_conversations`, `notifications` (chat_messages sudah di v4).
- RLS policy baru: `notif_read_own` di `notifications`.

#### Endpoint Baru

| Endpoint | Keterangan |
|---|---|
| `POST   /api/donations/:pointId/participants` | Donatur tekan Berangkat |
| `DELETE /api/donations/:pointId/participants/me` | Donatur batalkan |
| `GET    /api/donations/:pointId/participants/me` | Cek partisipasi sendiri |
| `GET    /api/donations/:pointId/participants` | List donatur (komunitas) |
| `PATCH  /api/donations/:pointId/participants/accept` | Bulk accept |
| `PATCH  /api/donations/:pointId/participants/complete` | Bulk complete + geo-fence + +50 poin |
| `PATCH  /api/donations/:pointId/progress` | Update progres → recompute urgency |
| `GET    /api/notifications` | List notifikasi user (pagination + filter unread) |
| `PATCH  /api/notifications/:id/read` | Tandai 1 dibaca |
| `PATCH  /api/notifications/read-all` | Tandai semua dibaca |
| `DELETE /api/notifications/:id` | Hapus 1 notifikasi |
| `GET    /api/users/points` | Total + riwayat Poin Donatur |

#### Diubah

- `POST /api/donations` — `urgency` di body **diabaikan**; selalu di-set `'Mendesak'`.
  Urgency lanjutan dihitung otomatis dari progres (lihat `src/utils/urgency.js`).
- `POST /api/community/posts/:id/like` — saat `liked=true`, kirim notifikasi `post_liked` ke author.
- `POST /api/community/posts/:id/comments` — kirim notifikasi `post_commented` ke author.

#### Breaking Changes untuk FE

- `POST /api/donations` body `urgency` tidak lagi efektif. Aman dikirim
  (tidak menyebabkan error), tapi nilainya di-override → FE bisa hapus field.
- `PATCH /api/donations/:id/status` masih ada, tapi flow utama transisi
  status diharapkan via `participants/accept` & `participants/complete`.

### v3.1 — 2026-05-28

Endpoint baru di bawah modul **Me — `/api/me`** untuk kebutuhan `UserActivityScreen` di Flutter. Tidak ada perubahan skema DB; murni penambahan REST endpoint berbasis token.

| Endpoint | Status |
|---|---|
| `GET /api/me/community/posts` | **Baru** — daftar postingan user (token-based) |
| `GET /api/me/community/likes` | **Baru** — daftar post yang di-like user + snapshot |
| `GET /api/me/community/comments` | **Baru** — daftar komentar user + snapshot post |

**Rasional desain (request tim FE):**

- Endpoint **berbasis token** (`/api/me/...`) — FE tidak perlu kirim `userId` manual.
- Response **sudah berisi snapshot** (`content`, `author_name`, `image_url`, `likes_count`, `comments_count`, dst.) agar FE tidak perlu 3-4 request tambahan per item.
- `post_id` dan `comment_id` konsisten bertipe **integer**; `created_at` selalu **ISO8601**.
- Pagination identik dengan endpoint community lain: `?page=&limit=` (default 1/10, maks 50).

### v3 — 2026-05-25

Migration: `database/migration_v3.sql`.
...
| 31 | GET | `/api/me/community/posts` | JWT | Semua |
| 32 | GET | `/api/me/community/likes` | JWT | Semua |
| 33 | GET | `/api/me/community/comments` | JWT | Semua |
| 34 | POST | `/api/donations/:pointId/participants` | JWT | Donatur |
| 35 | DELETE | `/api/donations/:pointId/participants/me` | JWT | Donatur |
| 36 | GET | `/api/donations/:pointId/participants/me` | JWT | Semua |
| 37 | GET | `/api/donations/:pointId/participants` | JWT | Komunitas (pemilik) |
| 38 | PATCH | `/api/donations/:pointId/participants/accept` | JWT | Komunitas (pemilik) |
| 39 | PATCH | `/api/donations/:pointId/participants/complete` | JWT | Komunitas (pemilik) |
| 40 | PATCH | `/api/donations/:pointId/progress` | JWT | Komunitas (pemilik) |
| 41 | GET | `/api/notifications` | JWT | Semua |
| 42 | PATCH | `/api/notifications/:id/read` | JWT | Semua |
